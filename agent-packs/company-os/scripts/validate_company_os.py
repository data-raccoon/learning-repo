...
def _validate_config(root: Path, errors: list[str]) -> None:
    path = root / "adapters" / "codex" / "config.toml"
    try:
    ...
    if agents.get("max_threads") != 6:
        errors.append("adapters/codex/config.toml must set agents.max_threads = 6")
    if agents.get("max_depth") != 1:
        errors.append("adapters/codex/config.toml must set agents.max_depth = 1")


def _validate_agents(root: Path, errors: list[str]) -> None:
    directory = root / "adapters" / "codex" / "agents"
    portable_directory = root / "agent-definitions"
    paths = sorted(directory.glob("*.toml"))
    portable_paths = sorted(portable_directory.glob("*.json"))
    if len(paths) != len(EXPECTED_AGENTS):
        errors.append(f"Expected {len(EXPECTED_AGENTS)} agent TOMLs, found {len(paths)}")
    if len(portable_paths) != len(EXPECTED_AGENTS):
        errors.append(f"Expected {len(EXPECTED_AGENTS)} portable agent definitions, found {len(portable_paths)}")
    ...
    for path in paths:
        ...
        portable_id = name.replace("_", "-")
        portable_path = portable_directory / f"{portable_id}.json"
        try:
            portable = json.loads(_read(portable_path, errors))
        except json.JSONDecodeError as exc:
            errors.append(f"Invalid JSON in {portable_path}: {exc}")
            continue
        if not isinstance(portable, dict) or set(portable) != {
            "schema_version", "id", "name", "description", "instructions", "permissions"
        }:
            errors.append(f"Portable agent {portable_id} has an invalid field set")
            continue
        if portable.get("schema_version") != 1 or portable.get("id") != portable_id:
            errors.append(f"Portable agent identity is invalid: {portable_id}")
        if portable_path.stem != portable_id:
            errors.append(f"Portable agent filename must match id: {portable_path.name}")
        if portable.get("description") != data.get("description"):
            errors.append(f"Codex adapter description drifted from portable agent {portable_id}")
        if portable.get("instructions", "").strip() != data.get("developer_instructions", "").strip():
            errors.append(f"Codex adapter instructions drifted from portable agent {portable_id}")
        permissions = portable.get("permissions")
        if not isinstance(permissions, dict) or permissions.get("workspace") != sandbox:
            errors.append(f"Codex adapter permissions drifted from portable agent {portable_id}")
...
def _validate_skills(root: Path, errors: list[str]) -> None:
    ...
    for skill_dir in sorted(path for path in directory.iterdir() if path.is_dir()) if directory.exists() else []:
        ...
        ui_path = root / "adapters" / "codex" / "skills" / skill_dir.name / "openai.yaml"
...
def validate(root: Path) -> list[str]:
    ...
    for path in (
        root / "AGENTS.md",
        root / "agent-pack.json",
        root / "adapters" / "vscode" / "settings.json",
