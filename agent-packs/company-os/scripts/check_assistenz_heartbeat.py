...
RUNTIME_ENV = "AI_LEARN_RUNTIME_ROOT"
...
def default_runtime_root() -> Path:
    ...
    if configured:
        return Path(configured).resolve() / "company-os"
    local_state = os.environ.get("LOCALAPPDATA")
    base = Path(local_state).resolve() if local_state else Path.home() / ".local" / "state"
    return base / "ai-learn" / "company-os"


def _contained_state_path(runtime_root: Path, state_path: Path) -> Path:
    runtime = (runtime_root / "assistenz-heartbeat").resolve()
...
def acknowledge(root: Path, runtime_root: Path | None = None) -> bool:
    root.resolve()
    runtime_root = (runtime_root or default_runtime_root()).resolve()
    state_path = _contained_state_path(runtime_root, runtime_root / "assistenz-heartbeat" / "state.json")
...
def check(root: Path, state_path: Path, today: date, persist: bool = True,
          runtime_root: Path | None = None) -> dict[str, list[dict[str, str]]]:
    root = root.resolve()
    runtime_root = (runtime_root or default_runtime_root()).resolve()
    state_path = _contained_state_path(runtime_root, state_path)
    pending_path = state_path.with_name("pending.json")
...
def main() -> int:
    ...
    if args.ack:
        ...
        acknowledge(args.root, args.runtime_root)
        return 0
    state_path = args.runtime_root / "assistenz-heartbeat" / "state.json"
    changes = check(
        args.root, state_path, args.today, persist=not args.dry_run,
