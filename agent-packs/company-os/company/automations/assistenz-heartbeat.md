...
The scanner checks active or overdue Work Orders and open risk-register entries.
It stores committed and pending deduplication checkpoints under the external
`AI_LEARN_RUNTIME_ROOT` (or the operating-system local state directory), never
inside the repository. An unchanged run produces no output.
...
Delegate repository triage to the `assistenz` agent. Do not use repository
write tools. The only permitted write is the scanner checkpoint below the
external Company-OS runtime root.
