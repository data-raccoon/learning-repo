...
- Heartbeat state lives outside the repository under `AI_LEARN_RUNTIME_ROOT`,
  or the operating-system local state directory when the variable is unset.
...
- The scanner may write only its contained external-runtime checkpoint state by
  default.
...
- [x] Checkpoint state is stored only below the configured external runtime root.
...
- `adapters/codex/agents/assistenz.toml`
...
| `$AI_LEARN_RUNTIME_ROOT/company-os/assistenz-heartbeat/state.json` (or the OS-local default) | scheduled `assistenz` run | local runtime only |
