...
class AssistenzHeartbeatTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.addCleanup(self.temporary_runtime.cleanup)
        self.root = Path(self.temporary.name)
        self.runtime = Path(self.temporary_runtime.name)
        ...
        self.state = self.runtime / "assistenz-heartbeat" / "state.json"
        self.today = date(2026, 7, 17)

    def test_first_run_reports_active_work_and_open_risk(self) -> None:
        changes = check(self.root, self.state, self.today, runtime_root=self.runtime)
    ...
    def test_unchanged_second_run_is_quiet(self) -> None:
        check(self.root, self.state, self.today, runtime_root=self.runtime)
        self.assertTrue(acknowledge(self.root, self.runtime))
        changes = check(self.root, self.state, self.today, runtime_root=self.runtime)
        self.assertEqual(changes, {"new": [], "changed": [], "resolved": []})

    def test_resolved_and_changed_findings_are_distinguished(self) -> None:
        check(self.root, self.state, self.today, runtime_root=self.runtime)
        acknowledge(self.root, self.runtime)
    ...
    def test_state_path_outside_runtime_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "Heartbeat state must be"):
            check(self.root, outside, self.today, runtime_root=self.runtime)
