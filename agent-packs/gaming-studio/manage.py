...
PROTECTED_TARGET_ROOTS = {".agents", ".git", "agent-packs", "local-models", "model-execution-harness"}
...
class GamingAgentsManager:
    ...
    def init(self, target_value: str, game_id: str) -> dict[str, Any]:
        ...
        jobs = state / "jobs"
        for directory in (context / "agent-definitions", context / "templates", jobs, state / "evidence"):
            directory.mkdir(parents=True, exist_ok=True)
        for source in sorted((self.package_root / "agent-definitions").glob("*.md")):
            shutil.copyfile(source, context / "agent-definitions" / source.name)
        ...
        director = {
            ...
            "context": [
                ...
                f"{STATE_NAME}/context/agent-definitions/game-director.md",
    ...
    def materialize_build(self, target_value: str) -> dict[str, Any]:
        ...
        creative = {
            ...
            "context": [f"{STATE_NAME}/context/agent-definitions/creative-producer.md", f"{STATE_NAME}/context/templates/handoff.md", *common_context],
        ...
        engineer = {
            ...
            "context": [f"{STATE_NAME}/context/agent-definitions/gameplay-engineer.md", f"{STATE_NAME}/context/templates/handoff.md", *common_context, *creative_expected],
        ...
        qa = {
            ...
            "context": [f"{STATE_NAME}/context/agent-definitions/qa-playtest.md", f"{STATE_NAME}/context/templates/qa-report.schema.json", *common_context, *creative_expected, *engineer_expected],
            "expected_artifacts": [],
    ...
    def validate(self, target_value: str) -> dict[str, Any]:
        ...
        if not state.is_dir():
            raise GamingAgentsError("target has not been initialized")
        game_id = self._game_id(target)
        source_root = self.workspace / "model-execution-harness" / "graph-runtime" / "src"
        if not source_root.is_dir():
            raise GamingAgentsError("graph runtime source is unavailable")
