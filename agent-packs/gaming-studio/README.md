# Gaming Studio agent pack

Model- and IDE-neutral agent definitions and job scaffolding for small-to-medium
playable game MVPs. This is an engine-neutral production workflow, not a game
engine and not a collection of running models.
...
```powershell
& "$env:USERPROFILE\.venvs\all\Scripts\python.exe" agent-packs\gaming-studio\manage.py init --target initiatives\my-game --id my-game
```

Edit `.game-agents/intent.md`, then execute the generated Director graph through the repository orchestrator:

```powershell
& "$env:USERPROFILE\.venvs\all\Scripts\python.exe" model-execution-harness\graph-runtime\orchestrate.py run-graph initiatives\my-game\.game-agents\jobs\01-game-director.graph.json
```
...
```powershell
& "$env:USERPROFILE\.venvs\all\Scripts\python.exe" agent-packs\gaming-studio\manage.py approve --target initiatives\my-game
& "$env:USERPROFILE\.venvs\all\Scripts\python.exe" agent-packs\gaming-studio\manage.py materialize-build --target initiatives\my-game
& "$env:USERPROFILE\.venvs\all\Scripts\python.exe" agent-packs\gaming-studio\manage.py validate --target initiatives\my-game
```

Run the generated build graph only after approval:

```powershell
& "$env:USERPROFILE\.venvs\all\Scripts\python.exe" model-execution-harness\graph-runtime\orchestrate.py run-graph initiatives\my-game\.game-agents\jobs\02-build-and-qa.graph.json
```
...
```powershell
& "$env:USERPROFILE\.venvs\all\Scripts\python.exe" -m unittest discover -s agent-packs\gaming-studio\tests -v
```
