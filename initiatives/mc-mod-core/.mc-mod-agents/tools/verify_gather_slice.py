...
SOURCES = [
    ...
    ROOT / "src/test/java/com/manacore/core/gather/SphericalGathererTest.java",
    ROOT / ".mc-mod-agents/tools/contracts/SphericalGathererContract.java",
]
...
def main() -> int:
    ...
    if not javac or not java or any(not source.is_file() for source in SOURCES):
        return 1
    with tempfile.TemporaryDirectory(prefix="mana-core-gather-test-") as output:
        ...
        for test_class in (
            "com.manacore.core.gather.SphericalGathererTest",
            "com.manacore.core.gather.SphericalGathererContract",
        ):
