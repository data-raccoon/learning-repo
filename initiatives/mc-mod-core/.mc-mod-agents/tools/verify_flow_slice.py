...
SOURCES = [
    ...
    ROOT / "src/test/java/com/manacore/core/flow/FlowEngineTest.java",
    ROOT / ".mc-mod-agents/tools/contracts/FlowEngineContract.java",
]
...
def main() -> int:
    ...
    if not javac or not java or any(not source.is_file() for source in SOURCES):
        return 1
    with tempfile.TemporaryDirectory(prefix="mana-core-flow-test-") as output:
        ...
        for test_class in (
            "com.manacore.core.flow.FlowEngineTest",
            "com.manacore.core.flow.FlowEngineContract",
        ):
